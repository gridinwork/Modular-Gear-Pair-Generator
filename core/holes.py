...
def prepare_gear_polygon(profile: np.ndarray, params: GearParams):
    ...
    try:
        ...
        if result.is_empty:
            raise ValueError("Отверстие слишком большое для этой шестерни. Уменьшите диаметр отверстия.")
        return result
    except ValueError:
        raise
...
def _keyway_cutter(bore_radius: float, key_width: float, key_depth: float, height: float) -> trimesh.Trimesh:
    box = trimesh.creation.box(extents=[key_width, key_depth * 2, height * 1.05])
    box.apply_translation([bore_radius + key_depth / 2, 0, 0])
    return box
